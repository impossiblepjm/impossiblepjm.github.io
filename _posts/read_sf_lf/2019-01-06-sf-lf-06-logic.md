...
* _propositions_: factual claims
  ...
  - implications (`P ‚Ü?Q`)
  - quantified propositions (`‚àÄ x, P`)
...
```coq
Definition injective {A B} (f : A ‚Ü?B) :=
  ‚àÄx y : A, f x = f y ‚Ü?x = y.
...
```coq
Check (‚àÄn:nat, S (pred n)).  (* not typeable *)
...
```coq
Lemma and_example2' :
  ‚àÄn m : nat, n = 0 ‚à?m = 0 ‚Ü?n + m = 0.
Proof.
  intros n m [Hn Hm]. (* = intro H. destruct H. *)
  rewrite Hn. rewrite Hm. reflexivity. Qed.  (* you could use only one *)
```

Instead of packing into conjunction `‚àÄn m : nat, n = 0 ‚à?m = 0 ‚Ü?n + m = 0.`
why not two separate premises?      `‚àÄn m : nat, n = 0 -> m = 0 ‚Ü?n + m = 0.` 
...
we could define `¬¨ P` ("not P") as `‚àÄ Q, P ‚Ü?Q.`. 

> Coq actually makes a slightly different (but equivalent) choice, defining `¬¨ P as P ‚Ü?False`, where `False` is a specific *contradictory proposition* defined in the standard library. 

```coq
Definition not (P:Prop) := P ‚Ü?False.
...
Proof.
  intros P contra.
  destruct contra.  Qed.  (* 0 cases to prove since ‚ä?is not provable. [inversion] also works *)
```
...
Proof.
  ...
  unfold not in HNA. apply HNA in HP.  (* HP : False, HNA : P -> False  ‚ä? HP: False  *)
  destruct HP.  Qed.                   (* destruct False.  *)
```

#### Tactic `exfalso.` 

> If you are trying to prove a goal that is nonsensical (e.g., the goal state is `false = true`), apply `ex_falso_quodlibet` to change the goal to `False`. This makes it easier to use assumptions of the form `¬¨P` that may be available in the context ‚Ä?in particular, assumptions of the form `x‚â†y`.
...
```coq
Definition iff (P Q : Prop) := (P ‚Ü?Q) ‚à?(Q ‚Ü?P).
Notation "P ‚Ü?Q" := (iff P Q)
...
To prove a statement of the form `‚àÉx, P`, we must show that `P` holds for some specific choice of value for `x`, 
...
but we can explicilty pass in to `plus_comm`, which is a forall type abstraction! (`Œî n m. (eq (n + m) (m + n))`) 
...
‚àÉf g, (‚àÄx, f x = g x) ‚Ü?f = g

‚àÉf g, (‚àÄx, f x = g x) ‚à?f != g    (* negation, consistent but not interesting... *)
```
...
Example even_42_prop : ‚àÉk, 42 = double k.
```

We wanna show there are _interchangable_.

```coq
Theorem even_bool_prop : ‚àÄn,
  evenb n = true ‚Ü?‚àÉk, n = double k.
```

> In view of this theorem, we say that the 
> boolean computation `evenb n` _reflects_ the truth of the proposition `‚à?k, n = double k`.

We can futhur general this to any equations representing as `bool` or `Prop`:

```coq
Theorem eqb_eq : ‚àÄn1 n2 : nat,
  n1 =? n2 = true ‚Ü?n1 = n2.
```
...
```coq
(* Logically *)
Example even_1000 : ‚àÉk, 1000 = double k.
Proof. ‚à?00. reflexivity. Qed.
