...
> "Algorithms are the computational content of proofs." â€”Robert Harper
...
1. `intros x` corresponds to `Î»x.` (or `Pi x.`??)
...
### `â†’` is degenerated `âˆ€` (`Pi`)

> Notice that both implication (`â†’`) and quantification (`âˆ€`) correspond to functions on evidence. 
> In fact, they are really the same thing: `â†’` is just a shorthand for a degenerate use of `âˆ€` where there is no dependency, i.e., no need to give a name to the type on the left-hand side of the arrow:
...
```coq
Definition some_nat_is_even : âˆƒn, even n :=
