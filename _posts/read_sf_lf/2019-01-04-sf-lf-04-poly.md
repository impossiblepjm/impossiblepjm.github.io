...
and it's **SystemFÏ‰**. the `list` here is a type-level small lambda, or **type operators**
...
Fixpoint repeat (X : Type) (x : X) (count : nat) : list X :=
  match count with
  | 0 â‡?nil X
  | S count' â‡?cons X x (repeat X x count')
  end.
```
...
Fixpoint repeat'' X x count : list X :=
  match count with
  | 0 â‡?nil _
  | S count' â‡?cons _ x (repeat'' _ x count')
  end.
...
Fixpoint repeat''' {X : Type} (x : X) (count : nat) : list X :=
  match count with
  | 0 â‡?nil
  | S count' â‡?cons x (repeat''' x count')
  end.
```
...
Fixpoint nth_error {X : Type} (l : list X) (n : nat) : option X :=
  match l with
  | [] â‡?None
  | a :: l' â‡?if n =? O then Some a else nth_error l' (pred n)
  end.
```
...
```coq
Definition doit3times {X:Type} (f:Xâ†’X) (n:X) : X :=
  f (f (f n)).
...
```coq
Fixpoint filter {X:Type} (test: Xâ†’bool) (l:list X)
                : (list X) :=
  match l with
  | [] â‡?[]
  | h :: t â‡?if test h then h :: (filter test t)
                        else filter test t
  end.
...
```coq
Fixpoint map {X Y: Type} (f:Xâ†’Y) (l:list X) : (list Y) :=
  match l with
  | [] â‡?[]
  | h :: t â‡?(f h) :: (map f t)
  end.
...
```coq
Definition option_map {X Y : Type} (f : X â†?Y) (xo : option X) : option Y :=
  match xo with
    | None â‡?None
    | Some x â‡?Some (f x)
  end.
...
```coq
Fixpoint fold {X Y: Type} (f: Xâ†’Yâ†’Y) (l: list X) (b: Y) : Y :=
  match l with
  | nil â‡?b
  | h :: t â‡?f h (fold f t b)
  end.
```
...
```coq
definition constfun {X: Type} (x: X) : natâ†’X :=
  fun (k:nat) â‡?x.
