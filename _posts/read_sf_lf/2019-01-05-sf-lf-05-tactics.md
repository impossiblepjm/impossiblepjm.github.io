...
> it requires `xRy1 ‚à?xRy2 ‚Ü?y1 = y2`  called "functional" or "univalent", "right-unique", or "deterministic"
> and also `‚àÄx ‚à?X, ‚àÉy ‚à?Y s.t. xRy`   called "left-total"
                ...
                argument    ‚Ü?    value
                    ...
                    X       ‚Ü?      Y
                 domain Âü? ‚Ü? co-domain Èô™Âüü      
           what can go into ‚Ü? what possibly come out

                  A ‚ä?X     ‚Ü? f(A) = {f(x) | x ‚à?A}
                            ‚Ü?    image
                            ‚Ü? what actually come out

    f‚Åª¬?B)={x ‚à?X|f(x) ‚à?B} ‚Ü?    B ‚ä?Y
                 preimage   ‚Ü?
...
- the image    of an element `x` of domain    ‚Ü? always single element of codomain (singleton set)
- the preimage of an element `y` of codomain  ‚Ü? may be empty, or one, or many!
  - `<= 1 ‚Ü?1` : injective   (left-unique)
  - `>= 1 ‚Ü?1` : surjective  (right-total)
  - `   1 ‚Ü?1` : bijective

Noted that the definition of "function" doesn't require "right-total"ity) until we have `surjective`.

graph = `[(x, f(x))]`, these points form a "curve", ÁúüÁöÑÊòØÂõæÂÉ?
...
- Injective:  `<= 1 ‚Ü?1` or `0, 1 ‚Ü?1` (distinctness) 
- Surjective: `>= 1 ‚Ü?1` (at least 1 in the domain)
- Bijective:  `   1 ‚Ü?1` (intersection of Inj and Surj, so only `1` preimage, _one-to-one correspondence_)
...
```coq
Theorem f_equal : ‚àÄ(A B : Type) (f: A ‚Ü?B) (x y: A),
  x = y ‚Ü?f x = f y.
Proof. 
  intros A B f x y eq. 
...
```coq
Theorem double_injective: ‚àÄn m,
        double n = double m ‚Ü?n = m.
```
