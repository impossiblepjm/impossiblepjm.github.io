...
Derive from the original **AT&T Unix**, Developed in the 1970s at **Bell Labs** (贝尔实验�?, initially intended for use inside the **Bell System**.

- #### Bell Labs
Bell �?AT&A 在那时已经是一家了，可以看到那时的通信公司真是一�?IT 公司呢�?
**C 语言也是 Bell Labs 的产�?*，从一开始就是为了用�?Unix 而设计出来的。所�?Unix （在 73 年用 C 重写）在高校流行后，C 语言也获得了广泛支持�?



AT&T licensed Unix to outside parties(第三�? from the late 1970s, leading to a variety of both **academic** (最有有名的 BSD ) and **commercial** (Microsoft Xenix, IBM AIX, SunOS Solaris)

- #### Xenix
微软 1979 年从 AT&A 授权来的 Unix OS，配合着 x86 成为当时最受欢迎的 Unix 发行版。后�?M$ �?IBM 合作开�?OS/2 操作系统后放弃，后来最终转�?**Windows NT**�?

- #### BSD
**Barkeley Software Distribution**, also called Berkeley Unix. Today the term "BSD" is used to refer to any of the BSD descendants(后代) which together form a branch of the family of Unix-like OS.(共同组成了一个分�?
	- **BSD 最大的贡献是在 BSD 中率先增加了虚拟存储器和 Internet 协议**，其 TCP/IP(IPv4 only) 代码仍然在现�?OS 上使用（ Microsoft Windows and most of the foundation of Apple's OS X and iOS �?
	- BSD 后来发展出了众多开源后代，包括 FreeBSD, OpenBSD, NetBSD 等等……很多闭源的 vendor Unix 也都�?BSD 衍生而来�?

- #### FreeBSD & Apple
FreeBSD 不但�?Open Source BSD 中占有率最高的，还直接影响�?Apple Inc : NeXT Computer 的团队在 FreeBSD 上衍生出�?NeXTSTEP 操作系统，这货后来在 Apple 时期演化成了 **Darwin** ，这个“达尔文”居然还是个开源系统，而且�?the Core of **Mac OS X** and **iOS**.
...
大家都知�?NeXT �?Steve Jobs �?forced out of Apple 后和 a few of his coworkers 创办的，所�?**NeXTSTEP 绝对是证�?Jobs 实力的作品�?* 
...
**所以说 Mac OS X 算是很正�?Unix 的了**

- #### POSIX
可移植操作系统接�? Portable Operating System Interface, is a family of standards specified by the IEEE from maintaining compatibility between OS, defines the API along with Command Line Shells and utility interfaces, for software comaptibility with variants of Unix and other OS.
...
* 最狭义�?Unix 单指 Bell Labs's Unix 
* 稍广义的 Unix 指代所�?Licensed Unix, 即通过�?SUS �?Unix-like ，比�?OS X
* 最广义�?Unix 即所�?Unix-like 系统，无论它是否通过过任�?SUS，包�?Linux，BSD Family �?
...
#### XNU Kernel
XNU, the acronym(首字母缩�? for ***X is Not Unix***, which is the **Computer OS Kernel** developed at Apple Inc since Dec 1996 for use in the Mac OS X and released as free open source software as part of Darwin.
...
严格来讲，术�?Linux 只表�?[Linux Kernel](http://en.wikipedia.org/wiki/Linux_kernel) 操作系统内核本身，比如说 Android is Based on Linux (Kernel). Linus 编写的也只是这一部分，一个免费的 Unix-like Kernel，并不属�?GNU Project 的一部分�?

但通常�?Linux 作为 Linux Kernel 与大量配合使用的 GNU Project Software Kit (包括 Bash, Lib, Compiler, 以及后期�?GUI etc) 所组合成的 OS 的统称。（包括各类 Distribution 发行版）
...
* [BSD Public License](http://en.wikipedia.org/wiki/BSD_licenses)
	* GPL 强制后续版本必须是自由软件，�?BSD 的后续可以选择继续开源或者封�?
...
Android Kernel 大概�?Linux Kernel 最得意的分支了，Android 也是 Linux 最流行的发行版。不过，也有一�?Google 工程师认�?Android is not Linux in the traditional Unix-like Linux distribution sense. 总之这类东西就算有各种协议也还是很难说清楚，在我理解�?Android Kernel 大概就是 fork Linux Kernel 之后改动和定制比较深的例子�?
...
ROM 在发展的过程中不断进化，从只读演变成了可编程可擦除，并最终演化成�?Flash  
...
所以我们平时习惯说�?ROM 其实还是来源于老单片机时代，那时的 ROM 真的是写了就很难（需要上电复位）、甚至无法修改，所以那时往 ROM 里烧下去的程序就被称�?firmware ，固件。久而久之，虽然技术发展了，固件仍然指代那些不常需要更新的软件，�?ROM 这个词也就这么沿用下来了�?
...
各类 Android ROM �?Android 词类下也都是属于 **Forks and distributions** 一类的�?

所以我说，其实各类 Android ROM 也好，fork Android 之流�?YunOS、FireOS 也好，改了多少东西，碰到多深�?codebase …�?*其实 ROM �?Distribution OS 的界限是很模糊的**，为什�?Android 就不可以是移动时代的 Linux ，为什�?Devlik/ART 就不能是移动时代�?GCC 呢？
...
虽然目前只是�?Web Thin Client OS ，但�?RoadMap 非常酷…�?
